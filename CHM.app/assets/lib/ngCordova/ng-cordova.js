...
(function () {
      ...
      .provider('$cordovaFile', [function () {
          ...
          this.$get = ['$q', '$window', '$cordovaFileError', function ($q, $window, $cordovaFileError) {
              ...
              return {
                              ...
                              fileSystem.getFile(file, { create: false }, function (fileEntry) {
                                  fileEntry.file(function (fileData) {
                                      ...
                                      reader.onloadend = function (evt) {
                                          if (evt.target.result !== undefined || evt.target.result !== null) {
                                              var objArrayBuffer = {};
                                              objArrayBuffer.FileName = file;
                                              objArrayBuffer.ArrayBufferValue = evt.target.result;
                                              q.resolve(objArrayBuffer);
                                          } else if (evt.target.error !== undefined || evt.target.error !== null) {
                                              q.reject(evt.target.error);
                                          } else {
                                              q.reject({ code: null, message: 'READER_ONLOADEND_ERR' });
                  ...
                  listDir: function (path, dirName, options) {
                   var q = $q.defer();
    ...
    angular.module('ngCordova.plugins.sqlite', [])
      ...
      .factory('$cordovaSQLite', ['$q', '$window', function ($q, $window) {
          ...
          var executeSql = function (db, query) {
              var q = $q.defer();
          ...
          return {
              ...
              multiSelect: function (db, queries) {
                  var q = $q.defer();
                  var deferredArr = [];
                  var results = [];
                  ...
                  db.transaction(function (tx) {
                      ...
                      for (var i = 0; i < queries.length; i++) {                          
                          deferredArr.push(executeSql(db,queries[i]).then(
                              function(res){
                                  results.push(res);
                              })
                            );                          
              ...
              //deleteRecord: function (db, query, binding) {
